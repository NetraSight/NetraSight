    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto" x-data="calendar" x-init="initCalendar">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-4">
            ...
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold"><span
            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                ...
                <!-- Create event button -->
                <button
                    class="btn bg-gray-900 text-gray-100 hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-800 dark:hover:bg-white">Create
                    Event</button>
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-4">
            ...
            <div class="mb-4 sm:mb-0 mr-2">
                <ul class="flex flex-wrap items-center -m-1">
                    <li class="m-1">
                        <button
                            class="btn-sm bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 text-gray-500 dark:text-gray-400">
                    ...
                    <li class="m-1">
                        <button
                            class="btn-sm bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 text-gray-500 dark:text-gray-400">
                            <div class="w-1 h-3.5 bg-orange-500 shrink-0"></div>
                    ...
                    <li class="m-1">
                        <button
                            class="btn-sm bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 text-orange-500">+Add
                            New</button>
                    </li>
                </ul>
            </div>
            ...
            <div class="flex flex-nowrap -space-x-px">
                <button
                    class="btn bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-700/60 text-orange-500 rounded-none first:rounded-l-lg last:rounded-r-lg">Month</button>
                <button
                    class="btn bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-700/60 hover:bg-gray-50 dark:hover:bg-gray-900 text-gray-600 dark:text-gray-300 rounded-none first:rounded-l-lg last:rounded-r-lg">Week</button>
                <button
                    class="btn bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-700/60 hover:bg-gray-50 dark:hover:bg-gray-900 text-gray-600 dark:text-gray-300 rounded-none first:rounded-l-lg last:rounded-r-lg">Day</button>
            </div>
        </div>

        <!-- Calendar table -->
        <div class="bg-white dark:bg-gray-950 rounded-xl shadow overflow-hidden" x-cloak>
            ...
            <div class="grid grid-cols-7 gap-px border-b border-gray-200 dark:border-gray-700/60">
                <template x-for="(day, index) in dayNames" :key="index">
                    <div class="px-1 py-3">
                        <div class="text-gray-500 text-sm font-medium text-center lg:hidden"
                            x-text="day.substring(0,3)"></div>
                        <div class="text-gray-500 dark:text-gray-400 text-sm font-medium text-center hidden lg:block"
                            x-text="day"></div>
                    </div>
                </template>
            </div>
            ...
            <div class="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700/60">
                ...
                <svg class="sr-only">
                    <defs>
                        <pattern id="stripes" patternUnits="userSpaceOnUse" width="5" height="5"
                            ...
                            <line class="stroke-current text-gray-200 dark:text-gray-700 opacity-50" x1="0"
                                y="0" x2="0" y2="5" stroke-width="2" />
                        </pattern>
                    </defs>
                </svg>
                <!-- Empty cells (previous month) -->
                <template x-for="blankday in startingBlankDays">
                    <div class="bg-gray-50 dark:bg-gray-950 h-20 sm:h-28 lg:h-36">
                ...
                <template x-for="(day, dayIndex) in daysInMonth" :key="dayIndex">
                    <div class="relative bg-white dark:bg-gray-950 h-20 sm:h-28 lg:h-36 overflow-hidden">
                            ...
                            <div class="grow flex flex-col relative p-0.5 sm:p-1.5 overflow-hidden">
                                <template x-for="event in getEvents(day)">
                                    <button class="relative w-full text-left mb-1">
                                        <div class="px-2 py-0.5 rounded-lg overflow-hidden"
                                            :class="{
                                                'text-white bg-sky-500': event.eventColor === 'sky',
                                                'text-white bg-orange-500': event.eventColor === 'indigo',
                                ...
                                <div class="absolute bottom-0 left-0 right-0 h-4 bg-gradient-to-t from-white dark:from-gray-800 to-transparent pointer-events-none"
                                ...
                                <template x-if="getEvents(day).length > 2">
                                    <button
                                        ...
                                        <span class="md:hidden">+</span><span
                                            x-text="getEvents(day).length - 2"></span> <span
                                            class="hidden md:inline">more</span>
                                    </button>
                                </template>
                                <!-- Day number -->
                                <button
                                    class="inline-flex ml-auto w-6 h-6 items-center justify-center text-xs sm:text-sm dark:text-gray-300 font-medium text-center rounded-full hover:bg-orange-100 dark:hover:bg-gray-600"
                                    :class="{ 'text-orange-500': isToday(day) }" x-text="day"></button>
                            </div>
                        </div>
                    </div>
                </template>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('calendar', () => ({
                ...
                events: [
                    // Previous month
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth() - 1, 8,
                            3),
                        eventEnd: new Date(new Date().getFullYear(), new Date().getMonth() - 1, 8,
                            7),
                        eventName: '⛱️ Relax for 2 at Marienbad',
                        eventColor: 'indigo'
                    },
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth() - 1,
                            12, 10),
                    ...
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth(), 7,
                            21),
                    ...
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth(), 9,
                            10),
                    ...
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth(), 11,
                            10),
                    ...
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth(), 14,
                            10),
                    ...
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth(), 21,
                            2),
                        eventEnd: '',
                        eventName: 'Pick up Marta at school',
                        eventColor: 'green'
                    ...
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth(), 22,
                            10),
                    ...
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth(), 23,
                            0),
                    ...
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth(), 25,
                            10),
                    ...
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth(), 29,
                            10),
                    ...
                    // Next month
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 2,
                            3),
                        eventEnd: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 2,
                            7),
                        eventName: '✍️ New Project (2)',
                        eventColor: 'yellow'
                    },
                    ...
                    },
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth() + 1,
                            25, 2),
                        eventEnd: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 25,
                            3),
                        eventName: 'Pick up Marta at school',
                        eventColor: 'green'
                    },
                    {
                        eventStart: new Date(new Date().getFullYear(), new Date().getMonth() + 1,
                            27, 21),
                        eventEnd: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 27,
                            22),
                        eventName: '⚽ 2024 - Semi-final',
                        eventColor: 'red'
                    },
                ...
                getEvents(date) {
                    return this.events.filter(e => new Date(e.eventStart).toDateString() === new Date(
                        this.year, this.month, date).toDateString());
                },
